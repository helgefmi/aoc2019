...
def print_area(area: Area, robot_coord: Optional[Coord] = None):
...
def pathing(area: Dict[Coord, Tile], from_coord: Coord, to_coord: Coord) -> Optional[List[Coord]]:
...
def oxidation_time(area: Area, end_coord: Coord) -> int:
    closed = set()
    next_oxygen = [end_coord]

    minutes = 0
    while True:
        new_oxygen = []
        for parent in next_oxygen:
            closed.add(parent)
            for child in get_adjacent_coords(parent):
                if child not in closed and area[child] == Tile.EMPTY:
                    new_oxygen.append(child)

        if not next_oxygen:
            break

        minutes += 1
        next_oxygen = new_oxygen
...
def compute(cts: str) -> int:
    ...
    assert robot.end_coord

    return oxidation_time(robot.area, robot.end_coord)
