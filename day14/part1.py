...
def compute(cts):
    ...
    inv = dict((k, 0) for k in rules.keys())
    inv['ORE'] = 0

    def recurse(prod_name, root=True):
        ores_needed = 0

        for other_name, other_num in rules[prod_name]['children'].items():
            if other_name == 'ORE':
                ores_needed += other_num
            else:
                while inv[other_name] < other_num:
                    new_ores = recurse(other_name, False)
                    ores_needed += new_ores
                inv[other_name] -= other_num

        if not root:
            inv[prod_name] += rules[prod_name]['makes']

        return ores_needed

    ores_needed = recurse('FUEL')
